...
def add_mask2_kernel(x_ptr, z_ptr, N0, B0: tl.constexpr):
    pid = tl.program_id(0)
    block_offsets = tl.arange(0, B0) + pid * B0
    mask = block_offsets < N0
    x_block = tl.load(x_ptr + block_offsets, mask=mask)
    new_x_block = x_block + 10
    tl.store(z_ptr + block_offsets, new_x_block, mask=mask)
...
def sum_kernel(x_ptr, z_ptr, N0, N1, T, B0: tl.constexpr, B1: tl.constexpr):
    ...
    for block in tl.range(0, T, B1): # which columns we are summing
      # we don't use block id anywhere, so we can omit cdiv op
...
def softmax_kernel(x_ptr, z_ptr, N0, N1, T, B0: tl.constexpr, B1: tl.constexpr):
    ...
    for block in tl.range(0, T, B1):
      ...
      denominator *= tl.exp2(log2_e * (old_maxx - maxx))
      denominator += tl.sum(tl.exp2(log2_e * (x_block - maxx)))
    for block in tl.range(0, T, B1):
      j_offsets = tl.arange(0, B1) + block
      j_mask = j_offsets < T
      ij_offsets = i_offsets[:, None] * T + j_offsets[None, :]
      ij_mask = i_mask[:, None] & j_mask[None, :]
      x_block = tl.load(x_ptr + ij_offsets, ij_mask)
      softmax = tl.exp2(log2_e * (x_block - maxx)) / denominator
